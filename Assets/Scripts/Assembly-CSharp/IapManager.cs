...
public class IapManager : Singleton<IapManager>
{
	...
	private BundleItem[] ParseBundleData(string json)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if (dictionary.ContainsKey("itemType") && dictionary.ContainsKey(key))
			{
				...
				if (int.TryParse(dictionary[key] as string, out result) && bundleItemType != BundleItem.BundleItemType.Undefined)
				{
	...
	private bool DeliverItem(InAppPurchaseItemType product)
	{
		...
		case InAppPurchaseItemType.AddTenDesserts:
		{
			...
			int maxExclusive = Singleton<GameManager>.Instance.gameData.m_desserts.Count - 1;
			int num = 8;
			for (int i = 0; i < num; i++)
			{
				GameProgress.AddDesserts(Singleton<GameManager>.Instance.gameData.m_desserts[UnityEngine.Random.Range(0, maxExclusive)].name, 1);
			}
