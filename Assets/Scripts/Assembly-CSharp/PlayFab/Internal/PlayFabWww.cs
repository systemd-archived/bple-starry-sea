		...
		private IEnumerator PostPlayFabApiCall(WWW www, Action<string> wwwSuccessCallback, Action<string> wwwErrorCallback)
		{
			...
			else
			{
				try
				{
					...
					bool num = bytes != null && bytes[0] == 31 && bytes[1] == 139;
					...
					if (num)
					{
						using GZipStream gZipStream = new GZipStream(new MemoryStream(bytes), CompressionMode.Decompress, leaveOpen: false);
						...
						string obj2 = new StreamReader(memoryStream).ReadToEnd();
				...
				{
					wwwErrorCallback("Unhandled error in PlayFabWWW: " + ex);
				}
			}
			www.Dispose();
		}
