...
public class ExtendedScrollList : Widget, WidgetListener
{
	...
	private float ScreenWidth()
	{
		if (fillScreenWidth)
		{
			return 20f * (float)Screen.width / (float)Screen.height - leftMargin - rightMargin;
		}
		if (screenWidth <= 0f)
		{
	...
	private void Start()
	{
		...
		if (fillScreenWidth && (float)horizontalCount * offset.x * 0.5f > 10f * (float)Screen.width / (float)Screen.height - leftMargin - rightMargin)
		{
			Vector3 localPosition = base.transform.localPosition;
			localPosition.x += 0.5f * leftMargin;
			localPosition.x -= 0.5f * rightMargin;
	...
	private void OnDrawGizmos()
	{
		...
		if (!Application.isPlaying && fillScreenWidth)
		{
			...
			float num2 = 13.333333f + rightMargin;
