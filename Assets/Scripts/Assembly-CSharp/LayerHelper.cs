...
public static class LayerHelper
{
	public static void SetSortingLayer(GameObject go, string layer, bool children)
	{
		...
		if (children)
		{
			...
			{
				SetSortingLayer(go.transform.GetChild(j).gameObject, layer, children);
			}
		}
	}
	...
	public static void SetOrderInLayer(GameObject go, int order, bool children)
	{
		...
		if (children)
		{
			for (int j = 0; j < go.transform.childCount; j++)
			{
				SetOrderInLayer(go.transform.GetChild(j).gameObject, order, children);
			}
		}
	}
	...
	public static void SetLayer(GameObject go, int layer, bool children)
	{
		...
		if (children)
		{
			...
			{
				SetLayer(go.transform.GetChild(i).gameObject, layer, children);
			}
		}
	}
}
