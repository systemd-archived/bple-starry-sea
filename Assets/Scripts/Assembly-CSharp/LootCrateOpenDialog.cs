...
public class LootCrateOpenDialog : WPFMonoBehaviour
{
	...
	public void AddLootCrate(LootCrateType lootCrateType, int amount, LootCrate.AnalyticData data, bool fromQueue = false, int xp = 0)
	{
		...
		for (int num8 = 0; num8 < randomRewards.Length; num8++)
		{
			int num9 = UnityEngine.Random.Range(0, randomRewards.Length);
			int num10 = randomRewards.Length;
			while (list2.Contains(num9) && num10 >= 0)
			{
				if (++num9 >= randomRewards.Length)
				{
					num9 = 0;
				}
				num10--;
			...
			list2.Add(num9);
			LootCrateRewards.SlotRewards reward = randomRewards[num9];
			switch (reward.Type)
			{
			case LootCrateRewards.Reward.Part:
			{
				...
				while (list.Contains(randomLootCrateRewardPartFromTier) && num12 > 0);
				list.Add(randomLootCrateRewardPartFromTier);
				if (CustomizationManager.IsPartUnlocked(randomLootCrateRewardPartFromTier))
				{
					num11 = Singleton<GameConfigurationManager>.Instance.GetValue<int>("part_salvage_rewards", randomLootCrateRewardPartFromTier.m_partTier.ToString());
					GameProgress.AddScrap(num11);
					num2 += num11;
					num7++;
				...
				componentInChildren.SetScrapIcon(num, scrapIcons[0], num11.ToString());
		...
		if (lootCrateRewardQueueElement != null && num > 0)
		{
			...
			int num13 = GameProgress.GetInt(lootCrateType.ToString() + "_crates_collected");
			GameProgress.SetInt(lootCrateType.ToString() + "_crates_collected", num13 + 1);
		}
