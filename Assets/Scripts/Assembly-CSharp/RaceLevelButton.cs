...
public class RaceLevelButton : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (levelIndex > 0)
		{
			...
			int num = GameProgress.GetInt(levelData.SceneName + "_stars");
			if (GameProgress.GetRaceLevelUnlocked(levelData.m_identifier) && num > 0)
			{
				flag = true;
			}
		...
		if ((flag3 && !flag4) || !base.transform.Find("Lock"))
		{
			...
			int num2 = GameProgress.GetInt(m_raceLevelSelector.FindLevelFile(m_raceLevelIdentifier) + "_stars");
