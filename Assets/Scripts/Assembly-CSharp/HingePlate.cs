...
public class HingePlate : BasePart
{
	public struct ConnectionData
	{
		...
		public ConnectionData(BasePart part, Rigidbody rigidbody, int angle)
		{
			Part = part;
			Rigidbody = rigidbody;
			Angle = angle;
	...
	public static void InitializeStatic()
	{
		...
		for (int j = 0; j < count; j++)
		{
			...
			foreach (ConnectionData leftPart in hingePlate.LeftParts)
			{
				if (dictionary.TryGetValue(leftPart.Part, out var value))
				{
					disjointSet.Union(j, value);
				}
			}
			foreach (ConnectionData rightPart in hingePlate.RightParts)
			{
				if (dictionary.TryGetValue(rightPart.Part, out var value2))
				{
	...
	private bool CanConnectToLeft(BasePart part, int x, int y, out bool flip, out int angle)
	{
		...
		int rotation2 = hingePlate.GetRotation();
		switch (rotation)
		{
		case 0:
			if (x == -1 && y == 0)
	...
	public void Create()
	{
		...
		for (int j = 0; j < 2; j++)
		{
			GameObject source = ((j == 0) ? plate : plate2);
			...
			foreach (ConnectionData item2 in obj)
			{
				...
				Joint item = CreateJoint(source, item2.Rigidbody.gameObject, type, item2.Angle, anchor);
				m_joints.Add(item);
			}
		}
	}
	...
	private Joint CreateJoint(GameObject source, GameObject target, JointType type, int angle, Vector3 anchor)
	{
		...
		if (customPartIndex == 7)
		{
			num = 4.7f;
		}
		float breakForce = (base.contraption.HasSuperGlue ? float.PositiveInfinity : (base.contraption.GetJointConnectionStrength(m_jointConnectionStrength) * INSettings.GetFloat(INFeature.ConnectionStrength) * num));
		switch (type)
		{
		case JointType.FixedJoint:
		{
			FixedJoint fixedJoint = source.AddComponent<FixedJoint>();
		...
		{
			angle %= 180;
			HingeJoint hingeJoint = source.AddComponent<HingeJoint>();
			...
			{
				min = (float)angle - num * 5f,
				max = (float)angle + num * 5f
			};
			return hingeJoint;
		}
		default:
