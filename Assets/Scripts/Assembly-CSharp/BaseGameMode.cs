...
public class BaseGameMode : GameMode
{
	...
	public override void OnDataLoadedStart()
	{
		...
		int num = GameProgress.GetInt("branded_reward_gift_time");
		int num2 = 86400;
		...
		{
			num2 = Singleton<GameConfigurationManager>.Instance.GetValue<int>("branded_reward_cooldown", "time");
		}
		if (num > 0 && Singleton<TimeManager>.Instance.CurrentEpochTime - num > num2)
		{
	...
	private void PlaceDesserts(bool forceFillAllPlaces = false)
	{
		...
		int maxExclusive = base.gameData.m_desserts.Count - 1;
		...
		for (int i = 0; i < num3; i++)
		{
			...
			GameObject gameObject2 = base.gameData.m_desserts[Random.Range(0, maxExclusive)];
			gameObject2 = ((num4 != i) ? base.gameData.m_desserts[Random.Range(0, maxExclusive)] : base.gameData.m_desserts[base.gameData.m_desserts.Count - 1]);
		...
		case LevelManager.GameState.Building:
		{
			...
			if (currentState == LevelManager.GameState.Running || currentState == LevelManager.GameState.PausedWhileRunning)
			{
				...
				if (retries == 3 && !levelManager.m_sandbox && !tutorialBookOpened)
				{
					...
					if (num < 3 && !GameProgress.IsLevelCompleted(Singleton<GameManager>.Instance.CurrentSceneName) && GameProgress.GetInt(TutorialPromotionCount) == 0)
					{
						openTutorial = true;
						num++;
