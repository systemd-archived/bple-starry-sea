...
public class CompactEpisodeSelector : MonoBehaviour
{
	...
	private void Layout()
	{
		...
		bool flag = UserSettings.GetBool(IsEpisodeToggledKey);
		if (flag)
		{
			int value = UserSettings.GetInt(CurrentSandboxEpisodeKey, 1);
			if (Mathf.Clamp(value, 0, m_episodesToggled.Count - 1) == 0 && m_preButtons != null)
			{
				SetCenterEpisode(m_preButtons[0]);
			}
			else
			{
				SetCenterEpisode(m_episodesToggled[Mathf.Clamp(value, 0, m_episodesToggled.Count - 1)]);
			}
		}
