...
public class Umbrella : BasePart
{
	...
	{
		if (INSettings.GetBool(INFeature.SpecialUmbrellas) && !this.IsSinglePart() && m_partTier == PartTier.Epic)
		{
			yield return new UIPartSliderButtonInfo(UIPartButtonType.Slider, 1, base.Type, (int)EffectDirection(), base.ConnectedComponent, new UIPartSliderButton.Range(m_deltaAngle * 57.29578f, 0f, -180f, 180f, 5f, 0.02f));
		}
	}
	...
	private void FixedUpdate()
	{
		...
		if (m_open)
		{
			...
			{
				float num6 = ((customPartIndex == 4) ? INUserSettings.Instance.StarSeaSettings.AnchorUmbrellaCoefficient : 1f);
				Vector3 velocity = base.rigidbody.velocity;
				float magnitude = velocity.magnitude;
				float num7 = ((magnitude > 4f * num6) ? (4f * num6 / magnitude) : 1f);
				base.rigidbody.AddForce(new Vector3((0f - velocity.x) * num7, (0f - velocity.y) * num7), ForceMode.VelocityChange);
				base.rigidbody.AddTorque(new Vector3(0f, 0f, -64f * num6 * base.rigidbody.angularVelocity.z), ForceMode.Acceleration);
				m_position = m_position * num7 + base.rigidbody.position * (1f - num7);
				Vector3 force = (m_position - base.rigidbody.position) / Time.deltaTime;
				force.z = 0f;
				float magnitude2 = force.magnitude;
				if (magnitude2 > 4f * num6)
				{
					force *= 4f * num6 / magnitude2;
				}
			...
			else if (flag && m_partTier == PartTier.Epic)
			{
				...
				float num9 = m_angle + m_deltaAngle;
				float num10 = num9 - Mathf.Atan2(up2.y, up2.x);
				...
				Vector3 vector = ((customPartIndex == 3) ? up2 : ((Vector3)new Vector2(Mathf.Cos(num9), Mathf.Sin(num9))));
			...
			else
			{
				...
				float num12 = Vector3.Dot(base.transform.up, vector2);
				float num13 = Mathf.Sign(Vector3.Cross(vector2, base.transform.right).z);
				float num14 = num13 * num12 * num12;
				if (Mathf.Abs(num14) > 60f)
				{
					num14 = 60f * Mathf.Sign(num14);
				}
				Vector3 up3 = base.transform.up;
				base.rigidbody.AddForceAtPosition(num14 * up3, position, ForceMode.Force);
				num12 = Vector3.Dot(base.transform.right, vector2);
				num13 = Mathf.Sign(Vector3.Cross(base.transform.up, vector2).z);
				num14 = 0.5f * num13 * num12 * num12;
				if (Mathf.Abs(num14) > 60f)
				{
					num14 = 60f * Mathf.Sign(num14);
				}
		...
		else
		{
			...
			float num15 = -0.2f * m_maximumForce;
			base.rigidbody.AddForce(num15 * base.transform.up, ForceMode.Force);
		}
		...
		if (m_open)
		{
			...
			if (INSettings.GetBool(INFeature.SpecialUmbrellas) && m_partTier != PartTier.Epic)
			{
				...
				m_angle = Mathf.Atan2(up4.y, up4.x);
				m_position = base.transform.position;
			}
		}
		else
