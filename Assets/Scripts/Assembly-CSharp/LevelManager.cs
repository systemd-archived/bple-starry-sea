...
public sealed class LevelManager : WPFMonoBehaviour
{
	...
	{
		float num = INSettings.GetFloat(INFeature.TerrainScale) * INUserSettings.Instance.LevelSceneSettings.TerrainScale;
		m_cameraLimits.topLeft *= num;
		m_cameraLimits.size *= num;
	...
	public bool LoadDessertsPlacement(GameObject dessertPlacesRoot)
	{
		...
		string text = GameProgress.GetString(key, null);
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			GameObject gameObject = WPFMonoBehaviour.gameData.m_desserts.Find((GameObject gameObject2) => gameObject2 != null && gameObject2.GetComponent<Dessert>().saveId == placeDessert[1]);
		...
		{
			GameObject dessert = array2[num2].dessert;
			Transform place = array2[num2].place;
			GameObject obj = UnityEngine.Object.Instantiate(dessert, place.position, place.rotation);
			obj.name = dessert.name;
