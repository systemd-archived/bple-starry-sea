...
public class CakeRaceMenu : WPFMonoBehaviour
{
	...
	private void OnEnable()
	{
		...
		if (infoLabel != null)
		{
			for (int num = 0; num < infoLabel.Length; num++)
			{
				if (infoLabel[num] != null)
				{
					infoLabel[num].gameObject.SetActive(active);
				}
			}
		}
		if (leaderboardButton != null)
		{
			...
			bool flag = GameProgress.GetBool("leaderboard_opened");
			leaderboardButton.transform.Find("NewContentTag").gameObject.SetActive(!IsTutorial && !flag);
		}
		UpdateCupIcon();
	}
	...
	private void OnRankFetched(GetLeaderboardAroundPlayerResult result)
	{
		...
		int num2 = GameProgress.GetInt("cake_race_highest_rank");
		if (num2 <= 0 || num < num2)
		{
		...
		{
			int num3 = GameProgress.GetInt(text);
			GameProgress.SetInt(text, num3 + 1);
		}
		...
		if (seasonSnoutCoinReward > 0)
		{
			...
			int num4 = GameProgress.GetInt("cake_race_coins_won");
			GameProgress.SetInt("cake_race_coins_won", num4 + seasonSnoutCoinReward);
		}
	...
	public void StartRace()
	{
		...
		{
			GameProgress.SetInt("cake_race_days_played", ++num);
		}
	...
	private void FindOpponent()
	{
		if (HasWeeklyData())
		{
			...
			int num = GameProgress.GetInt(key);
			int num2 = GameProgress.GetInt(key2);
			Singleton<PlayFabManager>.Instance.MatchMaking.FindOpponentReplay(GetWeeklyTrackIndex(), Singleton<PlayerProgress>.Instance.Level, num - num2, OnReplayFetched);
		}
		else
		{
			SetInfoLabel("Error fetching tracks");
