...
public class LevelLoader : MonoBehaviour
{
	...
	private Color ReadColor(BinaryReader reader)
	{
		...
		result.r = (float)((num >> 24) & 0xFF) * 0.003921569f;
		result.g = (float)((num >> 16) & 0xFF) * 0.003921569f;
		result.b = (float)((num >> 8) & 0xFF) * 0.003921569f;
		result.a = (float)(num & 0xFF) * 0.003921569f;
		return result;
	}
	...
	private void ReadData(GameObject obj, BinaryReader reader)
	{
		...
		obj.transform.position = new Vector3(position.x * num, position.y * num, position.z);
		obj.transform.localScale = new Vector3(localScale.x * num, localScale.y * num, localScale.z);
	}
