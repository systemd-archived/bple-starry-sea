...
public class InGameBuildMenu : WPFMonoBehaviour
{
	...
	private void Awake()
	{
		...
		if (INUserSettings.Instance.StarSeaSettings.ToolBookEnabled)
		{
			GameObject toolBookObj = new GameObject("ToolBookButton");
			toolBookObj.transform.parent = base.transform;
			float aspect = (float)Screen.width / (float)Screen.height * 10f;
			toolBookObj.transform.localPosition = new Vector3(aspect - 4f, 4f, 0f);
			toolBookObj.AddComponent<ToolBookButton>();
		}
	}
	...
	private void OnEnable()
	{
		...
		superBluePrintButton.SetUsedState(flag);
		SetSuperAutoBuildAvailable(flag);
