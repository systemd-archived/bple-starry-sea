...
public class SpotLight : BasePart
{
	...
	public override Direction EffectDirection()
	{
		...
		{
			return BasePart.Rotate8(Direction.Up, BasePart.GetIntegralAngle(m_gridRotation) / 45);
		}
		return base.EffectDirection();
	}
	...
	public override void Initialize()
	{
		if (INSettings.GetBool(INFeature.LightSystem) && this.IsEntityLight())
		{
			...
			if (EntityLight != null)
			{
				...
				if (lightSource != null && lightSource.lightTransform != null)
				{
					vector = lightSource.lightTransform.right;
				}
				else if (m_lightConeContainer != null)
				{
					vector = m_lightConeContainer.transform.right;
				}
				float num = EntityLight.Length;
				Vector3 position = base.transform.position;
				foreach (TimeBomb timeBomb in Resources.FindObjectsOfTypeAll<TimeBomb>())
				{
					if (timeBomb == null)
					{
						continue;
					}
					BasePart component = timeBomb.GetComponent<BasePart>();
					if (component == null || component.contraption == null || timeBomb.transform.IsChildOf(base.transform))
					{
						continue;
					}
					Vector3 vector2 = timeBomb.transform.position - position;
					float magnitude = vector2.magnitude;
					if (Vector3.Dot(vector, vector2 / magnitude) > 0.9999f && magnitude < num)
					{
						num = magnitude;
					}
				}
				EntityLight.Length = num;
			}
		}
	}
}
