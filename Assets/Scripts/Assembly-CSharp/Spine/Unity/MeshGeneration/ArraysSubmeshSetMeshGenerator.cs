	...
	public class ArraysSubmeshSetMeshGenerator : ArraysMeshGenerator, ISubmeshSetMeshGenerator
	{
		...
		public MeshAndMaterials GenerateMesh(ExposedList<SubmeshInstruction> instructions, int startSubmesh, int endSubmesh)
		{
			...
			for (int m = 0; m < count; m++)
			{
				...
				if (flag3)
				{
					SubmeshTriangleBuffer obj = submeshBuffers.Items[m];
					bool isLastSubmesh = m == count - 1;
					ArraysMeshGenerator.FillTriangles(ref obj.triangles, submeshInstruction2.skeleton, submeshInstruction2.triangleCount, submeshInstruction2.firstVertexIndex, submeshInstruction2.startSlot, submeshInstruction2.endSlot, isLastSubmesh);
					obj.triangleCount = submeshInstruction2.triangleCount;
					obj.firstVertex = submeshInstruction2.firstVertexIndex;
				}
			}
			if (flag3)
			{
			...
			if (addTangents)
			{
				...
				{
					SubmeshTriangleBuffer submeshTriangleBuffer = submeshBuffers.Items[num2];
					ArraysMeshGenerator.SolveTangents2DTriangles(tempTanBuffer, submeshTriangleBuffer.triangles, submeshTriangleBuffer.triangleCount, meshVertices, meshUVs, num);
				}
