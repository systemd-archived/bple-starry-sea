...
namespace Spine.Unity.MeshGeneration
{
	public class ArraysSubmeshedMeshGenerator : ArraysMeshGenerator, ISubmeshedMeshGenerator
	{
		...
		public MeshAndMaterials GenerateMesh(SubmeshedMeshInstruction meshInstructions)
		{
			...
			bool num = ArraysMeshGenerator.EnsureSize(vertexCount, ref meshVertices, ref meshUVs, ref meshColors32);
			...
			for (int i = 0; i < count; i++)
			{
				...
				ArraysMeshGenerator.FillVerts(skeleton, startSlot, endSlot, zSpacing, base.PremultiplyVertexColors, verts, meshUVs, meshColors32, ref vertexIndex, ref attachmentVertexBuffer, ref boundsMin, ref boundsMax);
				if (flag2)
				{
					SubmeshTriangleBuffer obj = submeshBuffers.Items[i];
					bool isLastSubmesh = i == count - 1;
					ArraysMeshGenerator.FillTriangles(ref obj.triangles, skeleton, submeshInstruction.triangleCount, submeshInstruction.firstVertexIndex, startSlot, endSlot, isLastSubmesh);
					obj.triangleCount = submeshInstruction.triangleCount;
					obj.firstVertex = submeshInstruction.firstVertexIndex;
				}
			}
			if (flag2)
			{
			...
			if (addTangents)
			{
				...
				int k = 0;
				for (int num2 = count; k < num2; k++)
				{
					SubmeshTriangleBuffer submeshTriangleBuffer = submeshBuffers.Items[k];
					ArraysMeshGenerator.SolveTangents2DTriangles(tempTanBuffer, submeshTriangleBuffer.triangles, submeshTriangleBuffer.triangleCount, meshVertices, meshUVs, vertexCount);
				}
