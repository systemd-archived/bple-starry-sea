...
namespace Spine.Unity.Modules
{
	...
	public class SkeletonRagdoll : MonoBehaviour
	{
		...
		{
			string item = b.data.name;
			if (stopBoneNames.Contains(item))
			{
				return;
			}
			...
			if (AttachBoundingBoxRagdollColliders(b).Count == 0)
			{
				float length = b.Data.Length;
				if (length == 0f)
				{
					gameObject.AddComponent<SphereCollider>().radius = thickness * 0.5f;
				}
				else
				{
			...
			gameObject.AddComponent<Rigidbody>().constraints = RigidbodyConstraints.FreezePositionZ;
		...
		private void UpdateSpineSkeleton(ISkeletonAnimation skeletonRenderer)
		{
			...
			foreach (KeyValuePair<Bone, Transform> item in boneTable)
			{
				...
				key.appliedRotation = Mathf.Lerp(key.appliedRotation, num, mix);
			}
		}
		...
		{
			List<Collider> list = new List<Collider>();
			GameObject gameObject = boneTable[b].gameObject;
			...
			foreach (Slot slot in skeleton.Slots)
			{
				...
				{
					if (item is BoundingBoxAttachment boundingBox && item.Name.ToLower().Contains("ragdoll"))
					{
						BoxCollider boxCollider = gameObject.AddComponent<BoxCollider>();
						Bounds boundingBoxBounds = SkeletonUtility.GetBoundingBoxBounds(boundingBox, thickness);
