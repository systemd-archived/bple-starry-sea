...
namespace Spine.Unity.Modules
{
	...
	public class SkeletonRagdoll2D : MonoBehaviour
	{
		...
		private void RecursivelyCreateBoneProxies(Bone b)
		{
			...
			if (AttachBoundingBoxRagdollColliders(b, gameObject, skeleton).Count == 0)
			{
				float length = b.data.length;
				if (length == 0f)
				{
					gameObject.AddComponent<CircleCollider2D>().radius = thickness * 0.5f;
				}
				else
				{
			...
			gameObject.AddComponent<Rigidbody2D>().gravityScale = gravityScale;
		...
		private static List<Collider2D> AttachBoundingBoxRagdollColliders(Bone b, GameObject go, Skeleton skeleton)
		{
			...
			foreach (Slot slot in skeleton.Slots)
			{
				...
				{
					if (item2 is BoundingBoxAttachment boundingBox && item2.Name.ToLower().Contains("ragdoll"))
					{
						PolygonCollider2D item = SkeletonUtility.AddBoundingBoxAsComponent(boundingBox, go, isTrigger: false);
						list.Add(item);
					}
				}
			}
