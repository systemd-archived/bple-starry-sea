	...
	public class SkeletonJson
	{
		...
		public SkeletonData ReadSkeletonData(TextReader reader)
		{
			...
			for (int count = linkedMeshes.Count; i < count; i++)
			{
				...
				Attachment attachment2 = (((linkedMesh.skin != null) ? skeletonData.FindSkin(linkedMesh.skin) : skeletonData.defaultSkin) ?? throw new Exception("Slot not found: " + linkedMesh.skin)).GetAttachment(linkedMesh.slotIndex, linkedMesh.parent);
				if (attachment2 == null)
				{
		...
		private Attachment ReadAttachment(Dictionary<string, object> map, Skin skin, int slotIndex, string name)
		{
			...
			case AttachmentType.Linkedmesh:
			{
				...
				if (text2 != null)
				{
					...
					linkedMeshes.Add(new LinkedMesh(meshAttachment, GetString(map, "skin", null), slotIndex, text2));
			...
			case AttachmentType.Path:
			{
				...
				int num = GetInt(map, "vertexCount", 0);
				ReadVertices(map, pathAttachment, num << 1);
		...
		private void ReadAnimation(Dictionary<string, object> map, string name, SkeletonData skeletonData)
		{
				...
				foreach (KeyValuePair<string, object> item7 in (Dictionary<string, object>)map["bones"])
				{
					...
					foreach (KeyValuePair<string, object> item8 in (Dictionary<string, object>)item7.Value)
					{
						...
						case "rotate":
						{
							...
							{
								rotateTimeline.SetFrame(num9, (float)item9["time"], (float)item9["angle"]);
								ReadCurve(item9, rotateTimeline, num9);
								num9++;
							}
						...
						case "shear":
						{
							...
							foreach (Dictionary<string, object> item10 in list2)
							{
								...
								float num7 = GetFloat(item10, "x", 0f);
								float num8 = GetFloat(item10, "y", 0f);
								translateTimeline.SetFrame(num6, time3, num7 * num5, num8 * num5);
								ReadCurve(item10, translateTimeline, num6);
								num6++;
							}
			...
			if (map.ContainsKey("ik"))
			{
				foreach (KeyValuePair<string, object> item11 in (Dictionary<string, object>)map["ik"])
				{
					...
					List<object> obj3 = (List<object>)item11.Value;
					IkConstraintTimeline ikConstraintTimeline = new IkConstraintTimeline(obj3.Count)
					{
						ikConstraintIndex = skeletonData.ikConstraints.IndexOf(item)
					};
					int num10 = 0;
					foreach (Dictionary<string, object> item12 in obj3)
					{
						float time4 = (float)item12["time"];
						float mix = GetFloat(item12, "mix", 1f);
						bool boolean = GetBoolean(item12, "bendPositive", defaultValue: true);
						ikConstraintTimeline.SetFrame(num10, time4, mix, boolean ? 1 : (-1));
						ReadCurve(item12, ikConstraintTimeline, num10);
						num10++;
					}
			...
			if (map.ContainsKey("transform"))
			{
				foreach (KeyValuePair<string, object> item13 in (Dictionary<string, object>)map["transform"])
				{
					...
					List<object> obj5 = (List<object>)item13.Value;
					TransformConstraintTimeline transformConstraintTimeline = new TransformConstraintTimeline(obj5.Count)
					{
						transformConstraintIndex = skeletonData.transformConstraints.IndexOf(item2)
					};
					int num11 = 0;
					foreach (Dictionary<string, object> item14 in obj5)
					{
						float time5 = (float)item14["time"];
						float rotateMix = GetFloat(item14, "rotateMix", 1f);
						float translateMix = GetFloat(item14, "translateMix", 1f);
						float scaleMix = GetFloat(item14, "scaleMix", 1f);
						float shearMix = GetFloat(item14, "shearMix", 1f);
						transformConstraintTimeline.SetFrame(num11, time5, rotateMix, translateMix, scaleMix, shearMix);
						ReadCurve(item14, transformConstraintTimeline, num11);
						num11++;
					}
			...
			if (map.ContainsKey("paths"))
			{
				...
				{
					int num12 = skeletonData.FindPathConstraintIndex(item15.Key);
					if (num12 == -1)
					{
					...
					foreach (KeyValuePair<string, object> item16 in (Dictionary<string, object>)item15.Value)
					{
						List<object> list3 = (List<object>)item16.Value;
						...
						case "spacing":
						{
							...
							{
								pathConstraintPositionTimeline = new PathConstraintPositionTimeline(list3.Count);
								if (pathConstraintData.positionMode == PositionMode.Fixed)
								{
									num14 = scale;
								}
							...
							int num15 = 0;
							foreach (Dictionary<string, object> item17 in list3)
							{
								pathConstraintPositionTimeline.SetFrame(num15, (float)item17["time"], GetFloat(item17, key5, 0f) * num14);
								ReadCurve(item17, pathConstraintPositionTimeline, num15);
								num15++;
							}
						...
						case "mix":
						{
							PathConstraintMixTimeline pathConstraintMixTimeline = new PathConstraintMixTimeline(list3.Count);
							pathConstraintMixTimeline.pathConstraintIndex = num12;
							int num13 = 0;
							foreach (Dictionary<string, object> item18 in list3)
							{
								pathConstraintMixTimeline.SetFrame(num13, (float)item18["time"], GetFloat(item18, "rotateMix", 1f), GetFloat(item18, "translateMix", 1f));
								ReadCurve(item18, pathConstraintMixTimeline, num13);
								num13++;
							}
			...
			if (map.ContainsKey("deform"))
			{
				foreach (KeyValuePair<string, object> item19 in (Dictionary<string, object>)map["deform"])
				{
					...
					{
						int num16 = skeletonData.FindSlotIndex(item20.Key);
						if (num16 == -1)
						{
						...
						{
							List<object> obj7 = (List<object>)item21.Value;
							VertexAttachment vertexAttachment = (VertexAttachment)skin.GetAttachment(num16, item21.Key);
							...
							DeformTimeline deformTimeline = new DeformTimeline(obj7.Count)
							{
								slotIndex = num16,
								attachment = vertexAttachment
							};
							int num18 = 0;
							foreach (Dictionary<string, object> item22 in obj7)
							{
								float[] array;
								...
								else
								{
									...
									int num19 = GetInt(item22, "offset", 0);
									float[] floatArray = GetFloatArray(item22, "vertices", 1f);
									Array.Copy(floatArray, 0, array, num19, floatArray.Length);
									if (scale != 1f)
									{
										int i = num19;
										for (int num20 = i + floatArray.Length; i < num20; i++)
										{
											array[i] *= scale;
										}
									}
								...
								deformTimeline.SetFrame(num18, (float)item22["time"], array);
								ReadCurve(item22, deformTimeline, num18);
								num18++;
							}
			...
			{
				List<object> obj8 = (List<object>)map[(!map.ContainsKey("drawOrder")) ? "draworder" : "drawOrder"];
				DrawOrderTimeline drawOrderTimeline = new DrawOrderTimeline(obj8.Count);
				...
				foreach (Dictionary<string, object> item23 in obj8)
				{
					...
					if (item23.ContainsKey("offsets"))
					{
						...
						for (int num22 = count - 1; num22 >= 0; num22--)
						{
							array2[num22] = -1;
						}
						List<object> list4 = (List<object>)item23["offsets"];
						int[] array3 = new int[count - list4.Count];
						int num23 = 0;
						int num24 = 0;
						foreach (Dictionary<string, object> item24 in list4)
						{
							int num25 = skeletonData.FindSlotIndex((string)item24["slot"]);
							if (num25 == -1)
							{
							...
							int num26 = num23 + (int)(float)item24["offset"];
							array2[num26] = num23++;
						}
						while (num23 < count)
						{
						...
						for (int num27 = count - 1; num27 >= 0; num27--)
						{
							if (array2[num27] == -1)
							{
								array2[num27] = array3[--num24];
							}
						}
					}
					drawOrderTimeline.SetFrame(num21++, (float)item23["time"], array2);
				}
			...
			{
				List<object> obj9 = (List<object>)map["events"];
				EventTimeline eventTimeline = new EventTimeline(obj9.Count);
				int num28 = 0;
				foreach (Dictionary<string, object> item25 in obj9)
				{
					...
					Event obj10 = new Event((float)item25["time"], eventData);
					obj10.Int = GetInt(item25, "int", eventData.Int);
					obj10.Float = GetFloat(item25, "float", eventData.Float);
					obj10.String = GetString(item25, "string", eventData.String);
					eventTimeline.SetFrame(num28++, obj10);
				}
