	...
	public class SkeletonBinary
	{
		...
		public SkeletonData ReadSkeletonData(Stream input)
		{
			...
			for (int count = linkedMeshes.Count; num14 < count; num14++)
			{
				...
				Attachment attachment = (((linkedMesh.skin != null) ? skeletonData.FindSkin(linkedMesh.skin) : skeletonData.DefaultSkin) ?? throw new Exception("Skin not found: " + linkedMesh.skin)).GetAttachment(linkedMesh.slotIndex, linkedMesh.parent);
				if (attachment == null)
				{
		...
		private Attachment ReadAttachment(Stream input, Skin skin, int slotIndex, string attachmentName, bool nonessential)
		{
			...
			case 0:
			{
				...
				regionAttachment.r = (float)((num13 & 0xFF000000u) >> 24) / 255f;
			...
			case 2:
			{
				...
				meshAttachment.r = (float)((num3 & 0xFF000000u) >> 24) / 255f;
		...
		private void ReadAnimation(string name, Stream input, SkeletonData skeletonData)
		{
			...
			for (int num2 = ReadVarint(input, optimizePositive: true); i < num2; i++)
			{
				...
				for (int num3 = ReadVarint(input, optimizePositive: true); j < num3; j++)
				{
					...
					case 0:
					{
						...
						{
							attachmentTimeline.SetFrame(l, ReadFloat(input), ReadString(input));
						}
					...
					case 1:
					{
						...
						for (int k = 0; k < num5; k++)
						{
							...
							if (k < num5 - 1)
							{
								ReadCurve(input, k, colorTimeline);
							}
						}
			...
			for (int num23 = ReadVarint(input, optimizePositive: true); num22 < num23; num22++)
			{
				...
				for (int num26 = ReadVarint(input, optimizePositive: true); num25 < num26; num25++)
				{
					...
					case 2:
					{
						...
						{
							pathConstraintMixTimeline.SetFrame(num31, ReadFloat(input), ReadFloat(input), ReadFloat(input));
							if (num31 < num28 - 1)
							{
								ReadCurve(input, num31, pathConstraintMixTimeline);
							}
						}
					...
					case 1:
					{
						...
						{
							pathConstraintPositionTimeline.SetFrame(num30, ReadFloat(input), ReadFloat(input) * num29);
							if (num30 < num28 - 1)
							{
								ReadCurve(input, num30, pathConstraintPositionTimeline);
							}
						}
			...
			int num56 = ReadVarint(input, optimizePositive: true);
			if (num56 > 0)
			{
				...
				for (int num57 = 0; num57 < num56; num57++)
				{
					...
					{
						Int = ReadVarint(input, optimizePositive: false),
						Float = ReadFloat(input),
						String = ((!ReadBoolean(input)) ? eventData.String : ReadString(input))
					});
				}
