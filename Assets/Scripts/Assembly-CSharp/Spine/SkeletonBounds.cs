...
namespace Spine
{
	public class SkeletonBounds
	{
		...
		public void Update(Skeleton skeleton, bool updateAabb)
		{
			...
			for (int j = 0; j < count; j++)
			{
				...
				if (slot.attachment is BoundingBoxAttachment boundingBoxAttachment)
				{
					...
					int num = (polygon.Count = boundingBoxAttachment.Vertices.Length);
		...
		private void aabbCompute()
		{
			...
			{
				Polygon obj = polygons.Items[i];
				float[] vertices = obj.Vertices;
				int j = 0;
				for (int count2 = obj.Count; j < count2; j += 2)
				{
