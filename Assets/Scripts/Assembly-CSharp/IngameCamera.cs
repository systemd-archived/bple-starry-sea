...
public class IngameCamera : WPFMonoBehaviour
{
	...
	private void Start()
	{
		...
		m_cameraMaxZoom = Mathf.Min(Mathf.Min(m_cameraLimits.size.x, m_cameraLimits.size.y) / 2f, 20f) * 10f * INSettings.GetFloat(INFeature.CameraMaxZoom) * INUserSettings.Instance.LevelSceneSettings.MaxCameraScale;
		m_cameraMinZoom = 7f * INUserSettings.Instance.LevelSceneSettings.MinCameraScale * 0.5f;
	...
	{
		if (INSelectionManager.IsInteracting)
		{
			isPanning = false;
	...
	private Texture2D m_fpsBg;

	private void OnGUI()
	{
		StarSeaSettings starSeaSettings = INUserSettings.Instance?.StarSeaSettings;
		if (starSeaSettings != null && !starSeaSettings.ShowFPS)
		{
			return;
		}
		m_fpsFrames++;
		...
		if (m_fpsTimer >= 0.5f)
		{
			m_fpsDisplay = (float)m_fpsFrames / m_fpsTimer;
			m_fpsFrames = 0;
			m_fpsTimer = 0f;
		}
		if (m_fpsBg == null)
		{
			m_fpsBg = new Texture2D(1, 1);
			m_fpsBg.SetPixel(0, 0, new Color(0f, 0f, 0f, 0.18f));
			m_fpsBg.Apply();
		}
		GUIStyle style = new GUIStyle();
		style.fontSize = 20;
		style.normal.textColor = new Color(1f, 1f, 1f, 0.22f);
		style.normal.background = m_fpsBg;
		GUI.Label(new Rect(Screen.width * 0.07f, 5f, 90f, 21f), "FPS: " + m_fpsDisplay.ToString("0.0"), style);
		if (starSeaSettings != null && starSeaSettings.AdaptivePhysicsFrame > 0)
		{
			int currentFrameRate = INDataDetector.CurrentFrameRate;
			if (currentFrameRate > 0)
			{
				GUI.Label(new Rect(Screen.width * 0.07f + 110f, 5f, 120f, 21f), "PFPS: " + currentFrameRate, style);
			}
		}
	}
}
