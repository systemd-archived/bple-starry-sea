...
public class PartListing : Widget
{
	...
	private void ReadPartOrder()
	{
		...
		for (int i = 0; i < 49; i++)
		{
			...
			{
				partOrder.Add(sortedPartType.ToPartType());
			}
		}
	}
	...
	{
		if (INSettings.GetBool(INFeature.FuelSystem) && type == BasePart.PartType.JetEngine)
		{
			return true;
		}
		switch (type)
	...
	private void CreatePartIcons(PartData data, float xPos, Transform parent, List<GameObject> newButtons)
	{
		for (int i = 0; i < PartTierCount - 1; i++)
		{
			for (int j = 0; j < data.RowWidth(); j++)
			{
				...
				Sprite icon = data.GetIcon(tier, index);
				GameObject icon2 = null;
				if (icon != null)
				{
					icon2 = UnityEngine.Object.Instantiate(icon.gameObject);
					icon2.transform.parent = bg.transform;
					icon2.transform.localPosition = new Vector3(0f, 0f, -0.1f);
					icon2.transform.localScale = Vector3.one * iconScale;
				}
	...
	private void RepositionFrameIcons(PartData data, float xPos)
	{
		...
		for (int num2 = 0; num2 < PartTierCount; num2++)
		{
			BasePart.PartTier key = (BasePart.PartTier)num2;
			if (data.partInstances != null && data.partInstances.ContainsKey(key))
			{
