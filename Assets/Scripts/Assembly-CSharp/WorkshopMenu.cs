...
public class WorkshopMenu : MonoBehaviour
{
	...
	public void AddScrap(int addScrapAmount = 0)
	{
		...
		int num2 = GameProgress.GetInt("Machine_scrap_amount");
		int num3 = num - num2;
		...
		{
			num4 = addScrapAmount;
			num5 = ((num4 + num2 >= value) ? ((num4 + num2 < value2) ? 1 : ((num4 + num2 < value3) ? 2 : ((!IsAlienMachine || num4 + num2 >= price) ? 3 : 2))) : 0);
			if (!IsAlienMachine && num4 + num2 > value3)
			{
				num4 = value3 - num2;
			}
			else if (IsAlienMachine && num4 + num2 > price)
			{
				num4 = price - num2;
			}
		}
		...
		else if (IsAlienMachine && !flag4 && num2 < price)
		{
		...
		if (num4 > 0 && num3 > 0)
		{
			if (num4 > num3)
			{
				ShowGetMoreScrapDialog(num4 - num3, nextTier);
				num4 = num3;
			}
			int partTierFromAmount = (int)GetPartTierFromAmount(num2);
			num2 += num4;
			GameProgress.SetInt("Machine_scrap_amount", num2);
			EventManager.Send(new CraftingMachineEvent(CraftingMachineAction.AddScrap, GameProgress.GetInt("Machine_scrap_amount")));
			num5 = (int)GetPartTierFromAmount(num2);
			if (num5 >= 0)
			{
				SetMachineAnimation(chainPullAnimationName, loop: false, queue: true, releaseAfterEnd: false);
				SetMachineAnimation(feedAnimationNames[num5]);
				SetMachineAnimation(idleAnimationNames[num5], loop: true, queue: true);
				if (num5 != partTierFromAmount)
				{
					SetMachineIdleSound(num5);
				}
			}
			StartCoroutine(MoveNuts(nutRootUpper, nutRootLower, num4, 0.2f));
		...
		else
		{
			...
			if (num4 > 0 && num3 <= 0)
			{
				ShowGetMoreScrapDialog(num4, nextTier);
			}
		}
	}
	...
	{
		int num = GameProgress.GetInt("Machine_scrap_amount");
		if (num >= removeAmount)
		{
			GameProgress.SetInt("Machine_scrap_amount", num - removeAmount);
	...
	public void ResetScrap(bool playAnimation = true)
	{
		...
		if (playAnimation)
		{
			...
			if (num > 0)
			{
				...
				StartCoroutine(MoveNuts(nutRootLower, nutRootUpper, num, 0.5f));
	...
	public void CraftSelectedPart()
	{
		...
		if ((bool)part && num > 0 && GameProgress.UseScrap(num))
		{
			...
			int num2 = GameProgress.GetInt(key);
			GameProgress.SetInt(key, num2 + 1);
