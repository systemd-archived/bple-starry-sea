...
public class MusicManager : MonoBehaviour
{
	...
	private Dictionary<string, AudioSource> m_audioPackMap;
	...
	private void ReceiveLoadingLevelEventOverride(LoadLevelEvent data)
	{
		...
		case GameManager.GameState.MainMenu:
			gameObject = commonAudio.MusicTheme;
			break;
		...
		{
			bool useLevelMusic = UserSettings.GetBool("UseLevelSelectionMusic", true);
			gameObject = useLevelMusic ? commonAudio.LevelSelectionMusic : commonAudio.MusicTheme;
			Debug.Log("[Music] LevelSelection(override): useLevelMusic=" + useLevelMusic);
			break;
		}
		case GameManager.GameState.Level:
			gameObject = (Singleton<GameManager>.Instance.OverrideBuildMusic ? Singleton<GameManager>.Instance.OverriddenBuildMusic : commonAudio.BuildMusic);
			break;
		case GameManager.GameState.KingPigFeeding:
			gameObject = ((Singleton<TimeManager>.IsInstantiated() && Singleton<TimeManager>.Instance.Initialized && Singleton<TimeManager>.Instance.CurrentTime.Month == 12) ? commonAudio.XmasThemeSong : commonAudio.FeedingMusic);
			break;
		case GameManager.GameState.WorkShop:
			gameObject = commonAudio.craftAmbience.gameObject;
			break;
		case GameManager.GameState.CakeRaceMenu:
			gameObject = commonAudio.CakeRaceTheme;
			break;
		default:
			gameObject = null;
			break;
		...
		if (gameObject != null)
		{
			...
			if (data.nextGameState == GameManager.GameState.MainMenu)
			{
				float entryVol = UserSettings.GetFloat("EntryMusicVolume", 1f);
				component.volume = entryVol;
				Debug.Log("[Music] MainMenu(override): EntryMusicVolume=" + entryVol);
			}
			StartMusic(component, 0.6f, 2.2f);
		}
	}

	public bool TryGetAudioPack(string name, out AudioSource audioSource)
	{
		return m_audioPackMap.TryGetValue(name, out audioSource);
	}
	...
	public void SetAudioPack(AudioClip audioClip, string name)
	{
		...
		if (audioClip == null)
		{
			if (m_audioPackMap.TryGetValue(name, out var value))
			{
				UnityEngine.Object.Destroy(value.gameObject);
				m_audioPackMap.Remove(name);
			}
			return;
		}
		GameObject obj = new GameObject("AudioPack_" + name);
		obj.transform.parent = base.transform;
		AudioSource audioSource = obj.AddComponent<AudioSource>();
		audioSource.clip = audioClip;
		audioSource.loop = true;
		audioSource.reverbZoneMix = 0f;
		if (m_audioPackMap.TryGetValue(name, out var value2))
		{
			UnityEngine.Object.Destroy(value2.gameObject);
		}
		m_audioPackMap[name] = audioSource;
	}
	...
	{
		if (TryGetAudioPack(music.name, out var audioSource))
		{
			music = audioSource;
		}
	...
	// 根据当前正在播放的音乐类别，应用"建造界面/游戏界面"独立静音开关：打开即对应 BGM 音量置 0（仍播放但无声）。
	private void ApplyCurrentVolume()
	{
		if (m_music == null || m_musicPrefab == null)
		{
			return;
		}
		AudioSource source = m_music.GetComponent<AudioSource>();
		float volume = m_musicPrefab.GetComponent<AudioSource>().volume * m_globalMusicVolume;
		GameObject buildMusic = (!Singleton<GameManager>.Instance.OverrideBuildMusic) ? commonAudio.BuildMusic : Singleton<GameManager>.Instance.OverriddenBuildMusic;
		GameObject inFlightMusic = (!Singleton<GameManager>.Instance.OverrideInFlightMusic) ? commonAudio.InFlightMusic : Singleton<GameManager>.Instance.OverriddenInFlightMusic;
		bool isBuild = (buildMusic != null && m_musicPrefab == buildMusic.gameObject);
		bool isInFlight = (inFlightMusic != null && m_musicPrefab == inFlightMusic.gameObject);
		if (isBuild && UserSettings.GetBool("MuteBuildMusic"))
		{
			volume = 0f;
		}
		if (isInFlight && UserSettings.GetBool("MuteInFlightMusic"))
		{
			volume = 0f;
		}
		// 建造/游戏(BGM)独立音量设置(0~1)叠加到对应音乐的最终音量上
		if (isBuild)
		{
			volume *= Mathf.Clamp01(UserSettings.GetFloat("BuildMusicVolume", 1f));
		}
		if (isInFlight)
		{
			volume *= Mathf.Clamp01(UserSettings.GetFloat("InFlightMusicVolume", 1f));
		}
		source.volume = volume;
	}

	// 供设置面板调用：切换建造/游戏界面静音开关后立即生效（即时静音/恢复当前正在播放的对应 BGM）。
	public void ApplyMusicMuteSetting()
	{
		ApplyCurrentVolume();
	}
	...
	private void ReceiveLoadingLevelEvent(LoadLevelEvent data)
	{
		...
		case GameManager.GameState.MainMenu:
		{
			...
			AudioSource menuSrc = musicTheme.GetComponent<AudioSource>();
			float entryVol2 = UserSettings.GetFloat("EntryMusicVolume", 1f);
			menuSrc.volume = entryVol2;
			StartMusic(menuSrc, 0f, 2.2f);
			Debug.Log("[Music] MainMenu(non-override): EntryMusicVolume=" + entryVol2);
			break;
		}
		case GameManager.GameState.EpisodeSelection:
		case GameManager.GameState.LevelSelection:
		case GameManager.GameState.SandboxLevelSelection:
		{
			bool useLevelMusic2 = UserSettings.GetBool("UseLevelSelectionMusic", true);
			GameObject levelSelectionMusic = useLevelMusic2 ? commonAudio.LevelSelectionMusic : commonAudio.MusicTheme;
			StartMusic(levelSelectionMusic.GetComponent<AudioSource>(), 0f, 2.2f);
			Debug.Log("[Music] LevelSelection(non-override): useLevelMusic=" + useLevelMusic2);
			break;
		}
		case GameManager.GameState.Level:
		{
