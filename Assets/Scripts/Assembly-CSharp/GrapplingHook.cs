...
public class GrapplingHook : BasePart
{
	...
	public override void Initialize()
	{
		...
		{
			int num3 = INSettings.GetInt(INFeature.MultipartGeneratorDetectionDistance);
			for (int i = 1; i <= num3; i++)
			{
				int x = m_coordX - num * i;
				int y = m_coordY - num2 * i;
				BasePart basePart2 = base.contraption.FindPartAt(x, y, this);
				if (basePart2 != null)
				{
		...
		else
		{
			...
			BasePart basePart3 = base.contraption.FindPartAt(x2, y2, this);
			if (basePart3 != null)
			{
	...
	private void Shoot()
	{
		if (INSettings.GetBool(INFeature.PartGenerator) && m_enclosedInto != null)
		{
			...
			{
				float num = INSettings.GetFloat(INFeature.PartGeneratorCoolingTime);
				if (!(templatePart != null) || !(Time.time > num + m_shootTime))
				{
					return;
				}
			...
			{
				int num2 = customPartIndex - 8;
				int num3 = INSettings.GetArray<int>(INFeature.MultipartGeneratorDistances)[num2];
				int num4 = INSettings.GetInt(INFeature.MultipartGeneratorDetectionDistance);
				{
					foreach (TemplatePartData templatePart2 in m_templateParts)
					{
						...
						{
							int x2 = templatePart.m_coordX - m_coordX + (num3 + num4) * x;
							int y2 = templatePart.m_coordY - m_coordY + (num3 + num4) * y;
			...
			if (templatePart != null)
			{
				int num5 = customPartIndex - 1;
				int num6 = INSettings.GetArray<int>(INFeature.PartGeneratorDistances)[num5];
				int x3 = num6 * x;
				int y3 = num6 * y;
	...
	private void Update()
	{
		if (m_State != State.WindedUp && (bool)base.rigidbody && (bool)m_GrapplingHookRigidbody)
		{
	...
	private void FixedUpdate()
	{
		...
		case State.WindedUp:
			...
			break;
		case State.Shoot:
			if (m_GrapplingHookScript.GetAttachType() != Hook.AttachType.None)
			{
	...
	private void UpdateTemplatePart()
	{
		if (!INSettings.GetBool(INFeature.PartGenerator) || !(m_enclosedInto != null))
		{
			return;
		...
		if (INSettings.GetBool(INFeature.MultipartGenerator) && this.IsMultipartGenerator())
		{
			for (int i = 0; i < m_templateParts.Count; i++)
			{
				...
				flag |= UpdateSingleTemplatePart(ref template);
	...
	private bool UpdateSingleTemplatePart(ref TemplatePartData template)
	{
		for (int i = 0; i < 2; i++)
		{
