...
public class LevelComplete : WPFMonoBehaviour
{
	...
	private bool ShowRewards(ref float delay)
	{
		...
		else if (m_unlockedRaceLevelPart != BasePart.PartType.Unknown)
		{
			delay += 0.5f;
			StartCoroutine(ShowUnlockedRaceLevelPart(delay + 1f));
			delay += 0.5f;
		}
		else if (m_unlockedPart != BasePart.PartType.Unknown)
		{
			delay += 0.5f;
	...
	private void CheckLevelEndAchievements()
	{
		...
		{
			int num = GameProgress.GetInt("Transported_Pumpkins");
			num++;
			GameProgress.SetInt("Transported_Pumpkins", num);
		...
		{
			int n2 = GameProgress.GetInt("Perfect_Completions");
			n2++;
			GameProgress.SetInt("Perfect_Completions", n2);
			Singleton<SocialGameManager>.Instance.TryReportAchievementProgress("grp.SKILLED_PILOT", 100.0, (int limit) => n2 >= limit);
		}
	...
	{
		SetNextLevelButtons();
		int stars = GameProgress.GetInt(Singleton<GameManager>.Instance.CurrentSceneName + "_stars");
		...
		{
			StartCoroutine(ShowLonelyPig(0f, stars, bPlayRewardAnim: false));
		}
	}
