...
public class Balloon : BasePart
{
	...
	{
		yield return new UIPartTriggerButtonInfo(UIPartButtonType.Trigger, 0, m_partType, (int)EffectDirection(), base.ConnectedComponent, HasOnOffToggle(), multiple: false);
	}
	...
	public override void Initialize()
	{
		...
		for (; i < num + 1; i++)
		{
			...
			m_connectedPart = base.contraption.FindPartAt(m_coordX - i * x, m_coordY - i * y, this);
		...
		if ((bool)m_connectedPart)
		{
			...
			float num2 = Vector3.Distance(m_connectedPart.transform.position, position) - 0.5f;
			float num3 = 0f;
			Vector3 vector;
	...
	public void FixedUpdate()
	{
		...
		float num = Physics.gravity.magnitude / 9.81f;
		float num2 = LimitForceForSpeed(m_force * num, vector);
		base.rigidbody.AddForce(num2 * vector, ForceMode.Force);
