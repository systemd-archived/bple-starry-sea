...
{
	private static INContraptionData s_toolBookClipboard;
	...
	private void ReceiveUIEvent(UIEvent data)
	{
		...
		case UIEvent.Type.OpenTutorial:
			goto case UIEvent.Type.ToolBookOpenTutorial;
		case UIEvent.Type.ToolBookOpenTutorial:
			...
			break;
		case UIEvent.Type.ToolBookCopy:
			if (INSelectionManager.Instance != null && INSelectionManager.Instance.CurrentState != INSelectionManager.State.Idle)
			{
				if (INSelectionManager.Instance.Clipboard != null)
				{
					BP.Feedback("BP> Selected parts copied");
				}
			}
			else if (ContraptionProto != null && ContraptionProto.Parts.Count > 0)
			{
				s_toolBookClipboard = INContraption.CopyContraption();
				BP.Feedback("BP> Copied " + ContraptionProto.Parts.Count + " parts to clipboard");
			}
			break;
		case UIEvent.Type.ToolBookPaste:
			if (INSelectionManager.Instance != null)
			{
				INSelectionManager.Instance.StartPastePreview();
			}
			else if (s_toolBookClipboard != null && s_toolBookClipboard.Units.Count > 0)
			{
				INContraption.PasteContraption(s_toolBookClipboard, 0, 0, absolute: true);
				BP.Feedback("BP> Pasted " + s_toolBookClipboard.Units.Count + " parts from clipboard");
			}
			break;
		case UIEvent.Type.ToolBookSelect:
			if (INSelectionManager.Instance != null)
			{
				if (INSelectionManager.Instance.CurrentState == INSelectionManager.State.Idle)
				{
					INSelectionManager.Instance.EnterSelectionMode();
				}
				else
				{
					INSelectionManager.Instance.ExitSelectionMode();
				}
			}
			break;
		case UIEvent.Type.ToolBookPasteConfirm:
			if (INSelectionManager.Instance != null)
			{
				INSelectionManager.Instance.ConfirmPaste();
			}
			break;
		case UIEvent.Type.ToolBookPasteCancel:
			if (INSelectionManager.Instance != null)
			{
				INSelectionManager.Instance.CancelPaste();
			}
			break;
		case UIEvent.Type.ToolBookUndo:
			if (INSelectionManager.Instance != null)
			{
				INSelectionManager.Instance.Undo();
			}
			break;
		case UIEvent.Type.ToolBookSaveClipboard:
			if (INSelectionManager.Instance != null)
			{
				INSelectionManager.Instance.SaveToClipboard();
			}
			break;
		case UIEvent.Type.ToolBookDeleteClipboard:
			break;
		case UIEvent.Type.ToolBookQuickPlace:
			if (ContraptionProto != null)
			{
				ContraptionProto.CreateAndSaveContraption(GetCurrentContraptionName());
				BP.Feedback("BP> Quick saved contraption \"" + GetCurrentContraptionName() + "\"");
			}
			break;
		...
		if (INUserSettings.Instance.ContraptionDataSettings.Enabled && data.type >= UIEvent.Type.LoadContraptionSlot4 && (int)data.type <= 68 + (slotCount - 4))
		{
			LoadContraptionFromSlot((int)(data.type - 68 + 3));
		}
	}
	...
	public void LoadContraptionSlot(int slotIndex)
	{
		LoadContraptionFromSlot(slotIndex);
	}
