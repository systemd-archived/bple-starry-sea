...
public class LootCrateSlots : MonoBehaviour
{
	...
	public static void AddLootCrateToFreeSlot(LootCrateType crateType)
	{
		for (int i = 0; i < SlotsAvailable; i++)
		{
			if (!IsSlotOccupied(i))
			{
				...
				int num = GameProgress.GetInt("loot_crates_added_to_slots");
				GameProgress.SetInt("loot_crates_added_to_slots", ++num);
				if (instance != null)
				{
	...
	{
		int num = GameProgress.GetInt("loot_crates_unlocked_from_slots");
		GameProgress.SetInt("loot_crates_unlocked_from_slots", ++num);
	}
