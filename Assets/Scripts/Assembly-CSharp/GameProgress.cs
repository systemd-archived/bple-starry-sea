...
public class GameProgress : MonoBehaviour
{
	...
	public static void AddPartBox(string levelName, string partBoxName)
	{
		...
		if (m_data.GetInt(key, 0) == 0)
		{
			...
			int num = m_data.GetInt(key2, 0);
			m_data.SetInt(key2, num + 1);
		}
	}
	...
	{
		int num = m_data.GetInt("Blueprints_Available", 0);
		num += count;
		m_data.SetInt("Blueprints_Available", num);
	}
	...
	{
		int num = m_data.GetInt("NightVisions_Available", 0);
		num += count;
		m_data.SetInt("NightVisions_Available", num);
	}
	...
	{
		int num = m_data.GetInt("SuperGlue_Available", 0);
		num += count;
		m_data.SetInt("SuperGlue_Available", num);
	}
	...
	{
		int num = m_data.GetInt("SuperMagnet_Available", 0);
		num += count;
		m_data.SetInt("SuperMagnet_Available", num);
	}
	...
	{
		int num = m_data.GetInt("TurboCharge_Available", 0);
		num += count;
		m_data.SetInt("TurboCharge_Available", num);
	}
	...
	public static void AddSnoutCoins(int count)
	{
		...
		{
			int num = m_data.GetInt("SnoutCoins", 0);
			num += count;
			m_data.SetInt("SnoutCoins", num);
		}
	}
	...
	{
		int num = m_data.GetInt("Scrap", 0);
		num += count;
		m_data.SetInt("Scrap", num);
		if (OnScrapAmountChanged != null)
		{
			OnScrapAmountChanged();
		}
	...
	{
		int num = m_data.GetInt("Ancient_Pigs_Revealed", 0);
		num += count;
		m_data.SetInt("Ancient_Pigs_Revealed", num);
	}
	...
	{
		string text = m_data.GetString("TimerIds", string.Empty);
	...
	{
		int num = m_data.GetInt(key, defaultValue);
		response?.Invoke(num);
		return num;
	}
	...
	{
		float num = m_data.GetFloat(key, defaultValue);
		response?.Invoke(num);
		return num;
	}
	...
	{
		string text = m_data.GetString(key, defaultValue);
	...
	{
		bool flag = m_data.GetBool(key, defaultValue);
	...
	private void UpgradeDataFormatVersion(bool isNewGameProgress = false)
	{
		...
		if (num == 5)
		{
			...
			foreach (Episode episodeLevel in instance.m_gameData.m_episodeLevels)
			{
				...
				{
					int value = GetInt(levelInfo.sceneName + "_stars");
					bool flag = LevelInfo.IsStarLevel(num3, num6);
					for (int l = 0; l < Mathf.Clamp(value, 0, 3); l++)
					{
						if (flag)
						{
							array2[l]++;
			...
			{
				int value2 = GetInt(level3.SceneName + "_stars");
				for (int m = 0; m < Mathf.Clamp(value2, 0, 3); m++)
				{
					array[m]++;
				}
			}
