...
public class RewardSystem : Singleton<RewardSystem>
{
	...
	private RewardDataHolder LoadSavedData()
	{
		...
		{
			Level = GetSavedRewardLevel(),
			RewardTime = GetSavedRewardTime(),
			ResetTime = GetSavedResetTime(),
			TimerMode = GetSavedTimerMode(),
			RandomSeed = GetSavedRandomSeed(),
			PendingRewardLevel = GetSavedPendingRewardLevel()
		};
	}
	...
	{
		RewardDataHolder result = new RewardDataHolder
		{
			ServerTime = time,
			ServerTimeUpdated = Mathf.RoundToInt(Time.realtimeSinceStartup)
		};
