...
public class CheatsUtility : WPFMonoBehaviour
{
	...
	private void OnGUI()
	{
		...
		DrawButton("1-star all levels", delegate
		{
			...
			{
				for (int i = 0; i < episodeLevel.LevelInfos.Count; i++)
				{
					SetStarsCompletion(episodeLevel.LevelInfos[i], 1);
				}
			}
		});
		DrawButton("3-stars all but one", delegate
		{
			...
			{
				int num = UnityEngine.Random.Range(0, episodeLevel2.LevelInfos.Count - 3);
				for (int i = 0; i < episodeLevel2.LevelInfos.Count - 2; i++)
				{
					if (i != num)
					{
						SetStarsCompletion(episodeLevel2.LevelInfos[i], 3);
					}
				}
			}
		});
		DrawButton("3-stars all", delegate
		{
			...
			{
				for (int i = 0; i < episodeLevel3.LevelInfos.Count; i++)
				{
					SetStarsCompletion(episodeLevel3.LevelInfos[i], 3);
				}
			}
		});
		DrawButton("Sandbox all starboxes", delegate
		{
			...
			{
				for (int i = 0; i < level.m_starBoxCount; i++)
				{
					if (i < 10)
					{
						GameProgress.AddSandboxStar(level.SceneName, "StarBox0" + i);
					}
					else
					{
						GameProgress.AddSandboxStar(level.SceneName, "StarBox" + i);
					}
				}
			}
		});
		DrawButton("Unlimited Sandbox Parts", delegate
		{
			...
			{
				if (value2 != BasePart.PartType.Unknown && value2 != BasePart.PartType.ObsoleteWheel && value2 != BasePart.PartType.JetEngine)
				{
		...
		DrawButton("Unlock & 3-star Race Levels except for last", delegate
		{
			...
			{
				GameProgress.SetInt(levels[i].m_identifier + "_stars", 3);
				GameProgress.SetBestTime(levels[i].SceneName, 10f);
				GameProgress.SetRaceLevelUnlocked(levels[i].m_identifier, unlocked: true);
			}
		});
