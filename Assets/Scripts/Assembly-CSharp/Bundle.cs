...
public class Bundle : MonoBehaviour
{
	...
	private IEnumerator LoadAssetBundle(string bundleId, Action onFinish)
	{
		...
		if (!string.IsNullOrEmpty(bundleLocation))
		{
			...
			{
				using (var req = UnityEngine.Networking.UnityWebRequestAssetBundle.GetAssetBundle(bundleLocation))
				{
					yield return req.SendWebRequest();
					if (req.result != UnityEngine.Networking.UnityWebRequest.Result.Success)
					{
						Debug.LogError($"Failed to load asset bundle: {bundleId} path={bundleLocation} error={req.error}");
						yield break;
					}
					assetBundle = UnityEngine.Networking.DownloadHandlerAssetBundle.GetContent(req);
				}
			}
			else
			{
