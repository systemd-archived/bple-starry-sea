...
namespace Ionic.Crc
{
	...
	public class CRC32
	{
		...
		{
			return (int)(crc32Table[(uint)(UIntPtr)((W ^ B) & 0xFF)] ^ (W >> 8));
		}
		...
		public void SlurpBlock(byte[] block, int offset, int count)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (reverseBits)
				{
					...
					_register = (_register << 8) ^ crc32Table[(uint)(UIntPtr)num2];
				}
				else
				{
		...
		public void UpdateCRC(byte b)
		{
			if (reverseBits)
			{
				...
				_register = (_register << 8) ^ crc32Table[(uint)(UIntPtr)num];
			}
			else
			{
		...
		public void UpdateCRC(byte b, int n)
		{
			while (n-- > 0)
			{
				...
				{
					uint num = (_register >> 24) ^ b;
					_register = (_register << 8) ^ crc32Table[(uint)(UIntPtr)((num < 0) ? (num + 256) : num)];
				}
				else
				{
					uint num2 = (_register & 0xFF) ^ b;
					_register = (_register >> 8) ^ crc32Table[(uint)(UIntPtr)((num2 < 0) ? (num2 + 256) : num2)];
				}
			}
		}
