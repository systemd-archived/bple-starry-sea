	...
	internal sealed class DeflateManager
	{
		...
		internal bool _tr_tally(int dist, int lc)
		{
			...
			else
			{
				matches++;
				dist--;
				short[] array2 = dyn_ltree;
				int num2 = (ZTree.LengthCode[lc] + InternalConstants.LITERALS + 1) * 2;
				array2[num2]++;
				short[] array3 = dyn_dtree;
			...
			if ((last_lit & 0x1FFF) == 0 && compressionLevel > CompressionLevel.Level2)
			{
				...
				{
					num4 = (int)(num4 + dyn_dtree[i * 2] * (5L + (long)ZTree.ExtraDistanceBits[i]));
				}
				num4 >>= 3;
		...
		internal void copy_block(int buf, int len, bool header)
		{
			...
			if (header)
			{
				...
				pending[pendingCount++] = (byte)(~(byte)len);
				pending[pendingCount++] = (byte)(~len >> 8);
			}
		...
		private void _fillWindow()
		{
			do
			{
				...
				else if (strstart >= w_size + w_size - MIN_LOOKAHEAD)
				{
					...
					do
					{
						int num5 = prev[--num3] & 0xFFFF;
						prev[num3] = (short)((num5 >= w_size) ? (num5 - w_size) : 0);
					}
					while (--num2 != 0);
					num += w_size;
				}
				...
				{
					int num2 = _codec.read_buf(window, strstart + lookahead, num);
					lookahead += num2;
