...
public class CakeRaceMode : GameMode
{
	...
	public override LevelManager.GameState SetGameState(LevelManager.GameState currentState, LevelManager.GameState newState)
	{
		...
		case LevelManager.GameState.Building:
		{
			...
			if (currentState == LevelManager.GameState.Running || currentState == LevelManager.GameState.PausedWhileRunning)
			{
				...
				{
					int num = GameProgress.GetInt("Tutorial_Promotion_Count");
					if (num < 3 && !GameProgress.IsLevelCompleted(base.gameManager.CurrentSceneName) && GameProgress.GetInt(TutorialPromotionCount) == 0)
					{
						openTutorial = true;
						num++;
						GameProgress.SetInt("Tutorial_Promotion_Count", num);
						GameProgress.SetInt(TutorialPromotionCount, 1);
					}
				}
				bool flag = GameProgress.GetBool(base.gameManager.CurrentSceneName + "_autobuild_available");
				bool flag2 = GameProgress.GetBool("PermanentBlueprint");
				if (!levelManager.m_sandbox && retries % 5 == 0 && AdvertisementHandler.GetRewardNativeTexture() != null)
				{
					int num2 = GameProgress.GetInt("branded_reward_gifts_today");
					int num3 = 2;
					...
					{
						num3 = Singleton<GameConfigurationManager>.Instance.GetValue<int>("branded_reward_gift_count", "count");
					}
					if (num2 < num3)
					{
						...
						GameProgress.SetInt("branded_reward_gifts_today", num2 + 1);
						openMechanicGift = true;
					}
				}
				else if (!levelManager.m_sandbox && retries == 5 && ((!flag2 && !levelManager.SuperBluePrintsAllowed) || (!flag && levelManager.SuperBluePrintsAllowed)) && Singleton<BuildCustomizationLoader>.Instance.IAPEnabled)
				{
					int num4 = GameProgress.GetInt("Mechanic_Promotion_Count");
					long num5 = Convert.ToInt64(GameProgress.GetString("Last_Mechanic_Promotion_Time_Binary", "0"));
					DateTime value = DateTime.Now;
					if (num5 != 0L)
					{
						value = DateTime.FromBinary(num5);
					}
					else
					{
						GameProgress.SetString("Last_Mechanic_Promotion_Time_Binary", value.ToBinary().ToString());
					}
					if ((DateTime.Now.Subtract(value).TotalMinutes > 1440.0 || num4 < 3) && !GameProgress.IsLevelCompleted(base.gameManager.CurrentSceneName) && GameProgress.GetInt(MechanicPromotionCount) == 0)
					{
						if (num4 < 1)
						{
						...
						num4++;
						GameProgress.SetInt("Mechanic_Promotion_Count", num4);
	...
	private void EndRace(int cakeIndex, bool waitToExplode = true)
	{
		...
		if (IsRaceOn && levelManager.gameState != LevelManager.GameState.CakeRaceCompleted)
		{
			...
			int num = GameProgress.GetInt("cake_race_total_wins");
			...
			int num2 = GameProgress.GetInt("cake_race_total_losses");
			...
			int num3 = GameProgress.GetInt(key2);
			int num4 = GameProgress.GetInt(key3);
			if (LocalPlayerIsWinner)
			{
				GameProgress.SetInt("cake_race_total_wins", ++num);
				GameProgress.SetInt(key2, ++num3);
			}
			else
			{
				GameProgress.SetInt("cake_race_total_losses", ++num2);
				GameProgress.SetInt(key3, ++num4);
			}
		}
	}
	...
	{
		int num = GameProgress.GetInt("cake_race_current_week");
		return $"{num:0000}_{Singleton<TimeManager>.Instance.ServerTime.Year:0000}";
	}
