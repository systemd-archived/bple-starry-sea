...
public class EpisodeSelector : MonoBehaviour
{
	...
	public int CurrentPage => Mathf.Clamp(Mathf.RoundToInt(m_scrollPivot.transform.localPosition.x / (0f - m_hudCamera.ScreenToWorldPoint(new Vector3(Screen.width, 0f, 0f)).x)), 0, m_pageCount);
	...
	private void Layout()
	{
		...
		Vector2 size = m_episodes[0].GetComponent<Sprite>().Size;
		float num2 = EdgeMargin + size.x / 2f + m_scrollButtonMargin;
		float y = 1f;
		...
		if (num4 < MinGap)
		{
			MinGap = 6.5f;
			EdgeMargin = 2f;
			num2 = EdgeMargin + size.x / 2f + m_scrollButtonMargin;
			y = 1f;
			vector = new Vector3((0f - num) / 2f + num2, y);
			num3 = num - 2f * num2;
			m_episodesPerPage = (int)(num3 / MinGap) + 1;
			if (m_episodesPerPage < count && gameData != null && m_episodesPerPage > gameData.m_episodeLevels.Count)
			{
				m_episodesPerPage = gameData.m_episodeLevels.Count;
			}
			vector.x = (0f - (float)(m_episodesPerPage - 1) * MinGap) / 2f;
			num4 = MinGap;
			m_pageCount = m_episodes.Count / m_episodesPerPage + ((m_episodes.Count % m_episodesPerPage != 0) ? 1 : 0);
		}
		else if (num4 > MaxGap)
		{
			m_pageCount = 1;
		...
		if (m_pageCount == 1)
		{
			...
			{
				m_episodes[i].transform.position = vector + Vector3.right * num4 * i;
			}
		}
		else
		{
			...
			while (num5 < count)
			{
				...
				{
					m_episodes[num5].transform.position = vector + Vector3.right * num4 * j;
					num5++;
					if (num5 >= count)
					{
						break;
